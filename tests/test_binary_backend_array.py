...
np_arr2 = np.array([[[1,2,3],[4,5,6]],[[10,11,12],[13,14,15]],[[20,21,22],[23,24,25]]]).astype('int8').T
np_arr3 = np.array([[[[1,2,3],[4,5,6]],[[10,11,12],[13,14,15]],[[20,21,22],[23,24,25]]],[[[101,102,103],[104,105,106]],[[111,112,113],[114,115,116]],[[120,121,122],[123,124,125]]],[[[31,32,33],[34,35,36]],[[41,42,43],[44,45,46]],[[50,51,52],[53,54,55]]],[[[61,62,63],[64,65,66]],[[71,72,73],[74,75,76]],[[70,71,72],[73,74,75]]]]).astype('int8').T
...
# however, the xarray adapter does not support Ellipsis and newaxis objects, so we don't test for them
@pytest.mark.parametrize("binfile,arr", [tuple([np_arr1]*2), tuple([np_arr2]*2), tuple([np_arr3]*2)], indirect=['binfile'])
def test_raw_indexing_method_integers(binfile, arr):
    ...
    for idx in idxlist:
        ...
        _idx[-1] = -idx[-1]
        assert test_arr._raw_indexing_method(tuple(_idx)) == arr[tuple(_idx)], f"Access {idx} at {arr} did not work"
...
def test_raw_indexing_method_slices(binfile, arr):
    ...
    idxlist = list(itertools.product(*map(list,map(range, map(lambda x: x//2, arr.shape)))))
    for idx in idxlist:
