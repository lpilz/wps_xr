...
@pytest.mark.parametrize("filename,dtype,dims", [("usgs/00001-01200.00001-01200", 'int8', ['x', 'y'])])
def test_open_dataset(filename, dtype, dims):
    _construct_index(os.path.join(test_files, os.path.dirname(filename)))
    ds = xr.open_dataset(os.path.join(test_files,filename), dtype=dtype, engine=BinaryBackend)
...
@pytest.mark.parametrize("filename,dtype,dims", [(Path("usgs/01201-02400.00001-01200"), 'int8', ['x', 'y'])])
...
def test_open_dataset_dask(filename, dtype, dims):
    _construct_index(os.path.join(test_files, os.path.dirname(filename)))
    ds = xr.open_dataset(os.path.join(test_files,filename), dtype=dtype, engine=BinaryBackend, chunks='auto')
    assert "foo" in ds.data_vars
    for dim in dims:
        assert dim in list(ds.dims.keys())
    assert ds.foo.dtype == dtype
